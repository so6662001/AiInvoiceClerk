...
AiInvoiceClerk 是一套面向钢贸企业的 **全自动智能开票系统**，包含 **销项开票智能体** 和 **收票智能体** 两大核心模块，支持客户自助申请开票、进项库存智能匹配、税负率/毛利区间自动调控、合同在线签署、诺诺网电子发票开具、进项发票自动获取与智能分配入库等全链路能力。
...
- [收票智能体业务流程设计](docs/09-receive-invoice-agent-design.md)
- [收票智能体数据库设计](docs/10-receive-invoice-database-design.md)
- [收票智能体API接口设计](docs/11-receive-invoice-api-design.md)
...
- 在线合同签署与线下盖章上传双通道
- 完整的审计追踪与合规保障
- 收票智能体：自动获取进项发票、智能匹配供应商与进货单、自动分配入库
- 进项库存管理粒度可配（具体规格 / 大品类 / 混合模式）
- 自动生成 ERP 进项发票、核销待收票、写入进项台账
